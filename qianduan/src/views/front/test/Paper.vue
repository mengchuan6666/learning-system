  ...
  <div style="margin-bottom: 100px; max-width: 60%; left: 20%; position:relative;">
   ...
   <div style="margin: 10px 0">
     ...
     <div style="margin: 20px; text-align: center">
       ...
       <el-button size="medium" type="warning" @click="$router.push('/front/testOnline?testType='+examId)">返回</el-button>
     </div>
   </div>
  </div>
</template>
    ...
    submitPaper() {
      this.request.post("/studentpaper", {examId: this.examId, answerList: this.questiones}).then(res => {
        if (res.code === '200') {
          ...
          this.$router.push('/front/testOnline?testType='+this.examId);
        } else {
